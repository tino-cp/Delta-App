...
function FindTimes()
  ...
  local results = AOBScan('?? ?? ?? 00 ?? 0? 00 00 FF FF FF FF 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 ?? ?? ?? ?? ?? 0? 00 00 ?? ?? ?? ?5 00 00 00 00 ?? ?? ?? 4? 00 00 00 00 ?? ?? ?? 4? ?? 0?', '-X-C+W', 1, '8')
  assert(results, 'aobscan failed')
  local addr = results[0]
  results.destroy()  
